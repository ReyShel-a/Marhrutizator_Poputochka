...
import { RouteModel, CreateRouteInput, UpdateRouteInput, SearchRoutesParams, NearbyRoutesParams } from '../types';
...
export class RouteRepository {
  ...
  async findById(id: string): Promise<RouteModel | null> {
    const result = await query(
  ...
  async update(id: string, data: UpdateRouteInput): Promise<RouteModel | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;
  ...
  async search(params: SearchRoutesParams): Promise<RouteModel[]> {
  ...
  async findNearby(params: NearbyRoutesParams): Promise<RouteModel[]> {
    const radius = params.radius || 20; // Default 20 km
    ...
    const result = await query(
      `SELECT * FROM routes
       ...
       LIMIT 50`,
      [params.lng, params.lat, radius * 1000]
    );
    
    return result.rows.map(row => this.mapRow(row));
  }

  private mapRow(row: any): RouteModel {
    return {
      id: row.id,
